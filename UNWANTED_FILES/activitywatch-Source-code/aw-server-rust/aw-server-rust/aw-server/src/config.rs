...
pub struct AWAuthConfig {
    /// Master API key for Bearer-token authentication.
    ...
    /// Per-node API keys, mapped by node identifier.
    /// Each node should use its own key for scoped access.
    /// Format: { "node_name": "api_key_for_that_node" }
    #[serde(default)]
...
/// Multi-node configuration for bucket isolation.
#[derive(Serialize, Deserialize, Default, Clone)]
pub struct AWNodeConfig {
    /// When true, each node's buckets are isolated by prefixing with node ID.
    /// For example, node "laptop" creates buckets as "laptop/aw-watcher-window".
    #[serde(default)]
    pub isolation: bool,
}
...
pub struct AWConfig {
    ...
    // TLS settings — serialised as [tls] section.
    #[serde(default)]
    pub tls: AWTlsConfig,

    // Multi-node settings — serialised as [node] section.
    #[serde(default)]
    pub node: AWNodeConfig,
...
impl Default for AWConfig {
    fn default() -> AWConfig {
        AWConfig {
            ...
            tls: AWTlsConfig::default(),
            node: AWNodeConfig::default(),
            cors: default_cors(),
