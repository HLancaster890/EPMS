...
class ActivityWatchClient:
    def __init__(
        ...
        Args:
            client_name: Name of this client/watcher
            testing: Use testing ports/config
            host: Server hostname (overrides config)
            port: Server port (overrides config)
            protocol: http or https
            node_id: Node identifier for multi-server setups. When set,
                     sends X-Node-Id header and prefixes bucket IDs.
        """
        ...
        if not self.server_api_key:
            self.server_api_key = os.environ.get("AW_API_KEY")
    ...
    def _headers(self, headers: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        ...
        if self.node_id:
            request_headers.setdefault("X-Node-Id", self.node_id)
    ...
    def get_event(
        ...
        effective_id = self._effective_bucket_id(bucket_id)
        endpoint = f"buckets/{effective_id}/events/{event_id}"
        try:
    ...
    def get_events(
        ...
        effective_id = self._effective_bucket_id(bucket_id)
        endpoint = f"buckets/{effective_id}/events"
    ...
    def get_eventcount(
        ...
        effective_id = self._effective_bucket_id(bucket_id)
        endpoint = f"buckets/{effective_id}/events/count"
    ...
    def heartbeat(
        ...
        effective_id = self._effective_bucket_id(bucket_id)
        endpoint = f"buckets/{effective_id}/heartbeat?pulsetime={pulsetime}"
    ...
    def _effective_bucket_id(self, bucket_id: str) -> str:
        """Prefix bucket ID with node_id if set."""
        result = f"{self.node_id}/{bucket_id}" if self.node_id else bucket_id
        return quote(result, safe="")

    def create_bucket(self, bucket_id: str, event_type: str, queued=False):
        # Queued path stores the original ID; _create_buckets will apply prefix later
        if queued:
            self.request_queue.register_bucket(bucket_id, event_type)
        else:
            effective_id = self._effective_bucket_id(bucket_id)
            endpoint = f"buckets/{effective_id}"
            data = {
    ...
    def delete_bucket(self, bucket_id: str, force: bool = False):
        effective_id = self._effective_bucket_id(bucket_id)
        self._delete(f"buckets/{effective_id}" + ("?force=1" if force else ""))
    ...
    def export_bucket(self, bucket_id) -> dict:
        effective_id = self._effective_bucket_id(bucket_id)
        return self._get(f"buckets/{effective_id}/export").json()
...
class RequestQueue(threading.Thread):
    ...
    def _create_buckets(self) -> None:
        for bucket in self._registered_buckets:
            effective_id = self.client._effective_bucket_id(bucket.id)
            endpoint = f"buckets/{effective_id}"
            data = {
